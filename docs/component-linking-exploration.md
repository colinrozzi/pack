# Component Linking Design Exploration

This document captures a design discussion about implementing component-to-component linking in Composite.

## Current State

Components can only import from the host - there's no way for Component A to call Component B directly. Component linking is listed as a Phase 4 TODO in the design roadmap.

## Design Decisions (Tentative)

| Decision | Choice | Rationale |
|----------|--------|-----------|
| Instantiation | Eager (all at once) | Simpler when packaging components together |
...
Track loaded components and their exports:
```rust
...
Each WASM component has isolated linear memory. Passing data between components requires:
- Reading bytes from source component's memory
- Writing bytes to target component's memory

### 4. Linker Integration
Extend `HostLinkerBuilder` to wire component exports as other components' imports.
...
```
Each component's memory layout:
+-------------------------------------+
| 0KB      Input Buffer (16KB)        |  <- host writes input here
+------------------------------------|
| 16KB     Output Buffer (32KB)       |  <- component writes output here
+-------------------------------------+
| 48KB     Heap (bump allocator)      |  <- component's working memory
+-------------------------------------+
```

Flow:
1. Host encodes Value to bytes
2. Host writes bytes into component's memory at offset 0
3. Component reads, processes, writes result at offset 16KB
4. Host reads bytes from component's memory

### Alternative: Host-Managed Buffers

Instead of each component owning I/O buffers, the host could manage a buffer pool:
...
Components interact via host calls:
```
host::read_input(buf_handle, offset, len) -> bytes
host::write_output(buf_handle, bytes)
```

**Advantage**: For component linking, pass the same buffer handle from A's output to B's input - no copy.

**Tradeoff**: Components can't do direct memory access; must call host for every buffer read/write.
...
- Component controls its own memory layout
...
If the Graph ABI wire format was also the in-memory representation, there's no serialization step. Component just returns a pointer to where the data already lives.
...
2. Implement component registry and dependency resolution
