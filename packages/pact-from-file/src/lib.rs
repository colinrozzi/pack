//! Regression fixture for the `pact!(from "path")` file form.
//!
//! Instead of an inline `pact! { ... }` block or a `pact/` directory, this crate
//! points the macro at a SHARED definition file that lives outside the crate
//! (`packages/shared-api.pact`). If this package builds, the file form resolves
...
packr_guest::pact!(from "../shared-api.pact");
