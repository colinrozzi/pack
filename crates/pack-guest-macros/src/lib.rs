...
/// Export a function with the Pack calling convention.
///
/// Transforms a Rust function into a WASM export. Parameters are automatically
/// extracted from the input Value tuple via `TryFrom<Value>`, and the return
/// value is converted via `Into<Value>`.
///
/// # Examples
///
/// ```ignore
/// // Value mode — raw Value in, Value out
/// #[export(name = "theater:simple/actor.init")]
/// fn init(input: Value) -> Value { ... }
///
/// // Typed mode — automatic conversion for all params
/// #[export(name = "theater:hello/actions.greet")]
/// fn greet(state: ActorState, name: String) -> Result<(ActorState, String), String> {
///     Ok((state, format!("Hello, {}!", name)))
/// }
/// ```
...
pub fn export(attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    // Detect Value mode (single Value parameter — pass through)
    ...
    // Generate call body — Value mode or Typed mode
    let call_body = if is_value_mode {
        ...
        // Typed mode — extract all params from input tuple, call, convert output
        ...
        quote! {
            let items = match value {
                ...
                other => pack_guest::__alloc::vec![other],
            };
        ...
        Some(custom_name) => {
            quote! {
                ...
                pub extern "C" fn #wrapper_fn_name(
                    ...
                    out_ptr_ptr: i32, out_len_ptr: i32,
                ) -> i32 {
