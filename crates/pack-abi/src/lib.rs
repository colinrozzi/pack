...
pub use value::{FromValue, KnownValueType, Rec, Value, ValueType};
...
impl core::fmt::Display for ConversionError {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            ...
            Self::ExpectedMap(got) => write!(f, "expected map, got {}", got),
            Self::ExpectedSet(got) => write!(f, "expected set, got {}", got),
    ...
    fn encode_graph(&self, encoder: &mut Encoder) -> Result<u32, AbiError> {
        match self {
            ...
            Value::Map {
                ...
                // Encode children first, in entry order (canonical by construction:
                // entries arrive key-sorted from the From<BTreeMap> path; the encoder
                // preserves order and does NOT re-sort — Value is not Ord).
            ...
            Value::Set { elem_type, items } => {
                // Canonical by construction: items arrive sorted from From<BTreeSet>;
...
fn node_children(node: &Node) -> Result<Vec<u32>, AbiError> {
    ...
    match node.kind {
        ...
        NodeKind::Map => {
            let _ = decode_value_type(&mut cursor)?; // key_type
            let _ = decode_value_type(&mut cursor)?; // value_type
            let count = cursor.read_u32()? as usize;
            for _ in 0..count {
                kids.push(cursor.read_u32()?); // key child
                kids.push(cursor.read_u32()?); // value child
            }
        }
        NodeKind::Set => {
            let _ = decode_value_type(&mut cursor)?;
    ...
    let value = match node.kind {
        ...
        NodeKind::Map => {
            ...
            for _ in 0..count {
                ...
                let k = decode_value(decoder, kc, shared, cache, visiting)?;
                let v = decode_value(decoder, vc, shared, cache, visiting)?;
                entries.push((k, v));
            }
            Value::Map {
                key_type,
